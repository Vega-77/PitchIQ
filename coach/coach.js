...
import { nowIndex } from '../assets/timeline.js?v=107';
import { renderShotMap, shotSummary } from '../assets/shot-map.js?v=107';
import { renderMatchVideo, teamMarks } from '../assets/match-video.js?v=107';
import {
    SAMPLE_NOTICE, isSample,
    samplePassEvents, samplePassMapping, sampleShapeGrids,
    sampleSubRoster, sampleSubEvents, sampleSubClock,
} from '../assets/sample-report.js?v=107';
import {
    playersByTrack, passingNetwork, foldEdges, strongestLink, networkNote,
} from '../assets/passing.js?v=107';
import { renderPassMap } from '../assets/pass-map.js?v=107';
import { mergeHeatmaps, orientedCentroid } from '../assets/heatmap.js?v=107';
import {
    seasonForms, formNote, MIN_FORM_POINTS, MIN_POINT_MINUTES,
} from '../assets/season.js?v=107';
import { renderForms } from '../assets/form-chart.js?v=107';
import {
...
} from '../assets/report.js?v=107';
import { CARD_COLOURS, describeEvent, timelineTone } from '../assets/events.js?v=107';
import { mountRail } from '../assets/rail.js?v=107';
import { mountPitchBackdrop, PITCH_LENGTH_M } from '../assets/pitch-backdrop.js?v=107';
import { videoKind } from '../assets/video.js?v=107';
import {
...
function renderPlayerRail() {
    playerRail ||= mountRail({
        ...
        heading: 'This season',
        facts: () => {
            const totals = state.openPlayer?.totals;
            if (!totals) return [];

            // Four facts, the same four the player's own rail carries. There
            // was a fifth for an afternoon — whether this student has signed in
            // — and it went: the pill beside their name says exactly that,
            // three inches away, and the point of these two rails is that a
            // coach and a player read one arrangement rather than two.
            return [
...
function renderPlayerTable(players) {
    ...
    setText('player-table-note', [
        ...
        minutesNote(state.match?.stats?.matchEndSource, {
            over: state.match?.status === 'full_time' || state.match?.finalized === true,
        }),
...
function init() {
    // review.js owns the verdicts; these two are how the rest of the page hears
    // about them. Set before any listener is wired, so no verdict can land
    // before there is somewhere for it to be reported.
    onReviewChange({ ledger: redrawShotViews, progress: renderMatchRail });
